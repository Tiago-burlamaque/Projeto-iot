...
def upload():
    ...
    return f"Upload realizado com sucesso. Arquivo salvo como: {caminho_arquivo}"

# Rota de recorte de mentirinha
...
def recortar():
    ...
    return f"""
    <h1>Dados recebido com sucesso</h1>
...
# Rota do recorte de Verdade

@app.route('/recortardeverdade', methods=["POST"])
def recortardeverdade():
    try:
        if "imagem" not in request.files:
        ...
        if not nome_saida:
            return jsonify({"Error": "Nome do arquivo de saída não informado"}), 400
